...
El objetivo de este programa es lograr encontrar reglas que dicten de manera eficaz cuales grupos van a colaborar entre si por medio de los patrones anteriores. Esto usando herramientas de reglas de asociación descritas poeteriormente en este documento. 

### Objetivos de minería de datos

Los objetivos de minera de datos son:

* Encontrar patrones de co-ocurrencia con una confianza acetable para el alcalde 
* Brindar una herramienta que permita predecir con cierta efectividad cuales grupos van a trabajar juntos segun patrones anteriores

### Criterios de éxito (desde la perspectiva de minería de datos)

Los criterios de éxtio serían: 
* Econtrar reglas que tengan una confianza de un 80%
* Proveer una lista que se pueda seguir facilmente a fin de poder determinar quien puede colaborar con quien
...
* *Tiempo_comunidad*: Tiempo en años que y 2 años), Medio (entre 3 y 9 años), Largo(10 o más años).
...
Los datos tienen `r sum(is.na(datos)) ` columnas con valores nulos. No es necesario realizar una limpieza de los mismos
...
El formato de todas las entradas es consistente con la definicion del diccionario de datos, por lo que no es requerido realizar arreglos a los datos. 
...
Es claro observar que la edad de los cuenstados esta distribuida de manera casi regular, sn embargo, hay pocas persona menores de 20 años y mayores de 56. 
...
Se puede ver que la encuesta esta bien distribuida, con casi un 50% de personas en cada sexo, si bien es cierto, hay un poco mas de encuestados del sexo femenino, la diferencia no es notable en la visualización. 
...
La gran mayoría de los encuestados llevan un largo tiempo viviendo en la comunidad, lo que puede lleger a ser un determinante a la hora de generar las reglas de asociación. 
...
En este caso, el 50% de los encuentados trbaja, mientras el otro 50% no lo hace. 
...
Es claro que el grupo mas popular es el de medio ambiente. Seguido por grupos de familia. El menos popular, es el grupo de política. 


# Preparación de los datos

## Selección de los datos
Se tomarán todas las columnas referentes a las personas, y a los grupos de las mismas. Simplement se elimina la variable del tiempo de respuesta de la encuesta. La misma no es relevante a los objetivos del proyecto, y mas bien es usada para medir el tiempo que tomó la encuesta en sí, y no los grupos a los que las personas pertenecen
...
En el caso de la edad, una diferencia de un año podría no ser relevante. A fin de poder hacer un manejo un poco mas exacto, se procede a crear un nuevo atributo, rango_edad, con 3 posibles valores: 
...
* Adulto mayor: Personas con mas de 60 años
...
Tal y como se menciona durante la exploración de los datos, se realiza un cambio de tipo a las variables numéricas a factores, asi pueden ser tratadas como tales. 

# Fase de modelado

## Selección de técnicas
A fin de poder obtener un conjunto de reglas, se utiliza el modelo "a priori" con difernetes valores de confianza y de soporte para obtener las reglas
...
Como parte del proceso, se realiza una selección de los parametros, tanto del soporte, como la confianza y el tamaño de las reglas en general, a fin de analizar las mismas
``` {r}
soporte <- 0.1
confianza <- 0.85
tamagnoRegla <- 10
...
Se procede a visualizar las reglas que se tienen, mayores que la confianza que se definió durante la generacion
...
En esta regla, se india que si una persona no forma parte de algún club relevante al medio ambiente, no formará parte tampoco a ningún club con caracter social

``` {r}
inspect(Top10[2])
```

En este caso, si una persona determinada no esta en un grupo de Hobbies, tampoco estará asociada a un curso de Clubes socuales

``` {r}
inspect(Top10[3])
```

Finalmente, esta regla indica que, con una confianza del 90%, nadie se va a unira grupos de política. Esto debido a que ha pocas personas en la tabla de transacciones que estan unidas al mismo, creando reglas que tienen niveles de confianza mas bajos que lo estipulado. 
...
Es de notar que la confianza de las tres reglas es bastante baja, con solo pa primer regla llegando al 90%. Las demas, pese a que son muy comunes, no sobrepasan el umbral del 85%