...
Sin embargo, los datos originales representan un desbalanceo de las clases. Esto es notable, tal y como se mostró en la exploración de los datos, pues la mayoria de los datos son 0s, es decir, no comprar el seguro. Para solucionar esto, se creará un nuevo dataframe, igualando la cantidad de personas que no compraron seguro a las que si lo hicieron, realizando undersampling
...
```
Es posible ver que hay variables que no tienen signigicancia, asi que se procede agenerar un nuevo modelo, sin las mismas, a fin de que se eliminen las variables no necesarias
```{r}
...
Vemos aun mas variables sin significancia, se procede a crear un nuevo modelo que las elimine
```{r}
...
Se aprecia que todas las varibales aportan significancia a la predicción. Se procederá con este modelo generado.
...
Se aprecia que el arbol de decisión no toma todas las variables en consideración
...
### Árbol de decisión
A continuación se evalua el árbol de decisión
