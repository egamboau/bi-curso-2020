...
A fin de conocer las correlaciones entre las variables numéricas: 
...
No se ven correlacines fuertes en las variables numéricas de los datos que se están analizando. 
...
Para este set de datos, no es necesario realizar limpiezas de estos, ni es necesaria la creación de nuevos atributos, pues los modelos se encargan de realizar operaciones sobre las variables categóricas.

Sin embargo, los datos originales representan un desbalanceo de las clases. Esto es notable, tal y como se mostró en la exploración de los datos, pues la mayoría de los datos son 0s, es decir, no comprar el seguro. Para solucionar esto, se creará un nuevo dataframe, igualando la cantidad de personas que no compraron seguro a las que si lo hicieron, realizando undersampling
...
# Modelado
Tal y como se menciona en la sección de técnicas a aplicar, se procede a aplicar dos modelos de clasificación a los datos.

## Regresión Logística
Se procede a crear un modelo de regresión con todas las variables, a fin de identificar cuales pueden ser más significativas en esta primera iteración. Nótese que se utiliza una regresión binomial pues la variable a predecir tiene dos valores posibles únicamente:

```{r}
modelo.logistica <- bayesglm(CARAVAN ~ . ,family = binomial, data=datos_sample, maxit=1000,drop.unused.levels   = FALSE)
summary(modelo.logistica)
```
Es posible ver que hay variables que no tienen significancia, así que se procede a generar un nuevo modelo, sin las mismas, a fin de que se eliminen las variables no necesarias
```{r}
modelo.logistica <- bayesglm(CARAVAN ~ MOSTYPE + MGEMOMV +  MGEMLEEF + MOSHOOFD + MGODRK + MGODOV + MGODGE + MRELGE + MRELOV + MFALLEEN + MFGEKIND + MOPLHOOG + MOPLLAAG + MBERHOOG + MBERMIDD + MBERARBG + MSKA + MSKB1 + MSKB2 + MSKC + MSKD + MAUT1 + MAUT2 + MAUT0 + MINKM30 + MINK4575 + MINK7512 + MINK123M + MKOOPKLA + PWAPART + PPERSAUT + PMOTSCO + PTRACTOR + PLEVEN + PWAOREG + PBRAND + PPLEZIER + APERSAUT + AMOTSCO + ATRACTOR + ABROM + ALEVEN + AWAOREG + APLEZIER,family = binomial, data=datos_sample, maxit=1000,drop.unused.levels   = FALSE)
summary(modelo.logistica)
```

En este caso, siguen existiendo variables sin significancia, se procede a continuar eliminando
```{r}
modelo.logistica <- bayesglm(CARAVAN ~ MOSTYPE + MGEMOMV +  MGEMLEEF + MOSHOOFD + MGODRK + MGODOV + MGODGE + MRELGE + MRELOV + MFALLEEN + MFGEKIND + MOPLHOOG + MOPLLAAG + MBERHOOG + MBERMIDD + MBERARBG + MSKA + MSKB1 + MSKB2 + MSKC + MSKD + MAUT1 + MAUT2 + MAUT0 + MINKM30 + MINK4575 + MINK7512 + MINK123M + MKOOPKLA + PWAPART + PPERSAUT + PMOTSCO + PTRACTOR + PLEVEN + PWAOREG + PBRAND + PPLEZIER + APERSAUT + AMOTSCO + ATRACTOR + ABROM  + AWAOREG + APLEZIER,family = binomial, data=datos_sample, maxit=1000,drop.unused.levels   = FALSE)
summary(modelo.logistica)
```

Vemos aún más variables sin significancia, se procede a crear un nuevo modelo que las elimine
```{r}
...
Se aprecia que todas las variables aportan significancia a la predicción. Se procederá con este modelo generado.

## Árbol de decisión
Un caso útil para este modelo, tal y como se describió anteriormente, es el árbol de decisión. Este permitirá que se siga un flujo para determinar quién comprará un seguro o no. Se implementa uno a continuación:
...
Se aprecia que el árbol de decisión no toma todas las variables en consideración
## Evaluación de los modelos

Primero, se cargan y transforman los datos de prueba suministrados: 
```{r}
...
Se obtienen las siguientes medidas respecto a los datos con el modelo de:
...
Finalmente, la curva ROC del modelo en cuestión: 
